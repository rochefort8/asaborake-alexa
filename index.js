...
const welcomeMessage = "日本人のこころ、百人一首を楽しみましょう。百句ある歌を、ランダムな順番で読み上げます。始めますか？";
...
const helpMessage ="百人一首の百句ある歌を、毎回異なる順番で読み上げます。上の句は一度、下の句は二度繰り返しで読み上げます。上の句からの読み上げを繰り返すには「もう一度」「もう一回」、次の句に移動するときは「次」と指示します。また、途中でやめることもできます。";
...
const newSessionHandler = {
	...
	this.handler.state = states.STARTMODE;
	let message = helpMessage + promptToStartMessage ;
...
const startHandlers = Alexa.CreateStateHandler(states.STARTMODE, {
	...
	readingOrderArray = new Array(100) ;
	tanka.getRandomOrder(readingOrderArray);	
...
const onReadingHandlers = Alexa.CreateStateHandler(states.ASKMODE, {
	...
	// Handle Yes intent.
	readingPosition++ ;	    	
	if (readingPosition < numberOfTotalReading - 1) {
	    helper.read(this,'yes');
	} else {
	    this.handler.state = states.STARTMODE;
    	    helper.readLast(this,'yes');
	}
	this.emit(':responseReady');
    },
    ...
    'AMAZON.HelpIntent': function () {
	let message = helpMessage + promptToSayYesNo ;
	this.response.speak(message).listen(message);
	this.emit(':responseReady');
    },
    'AMAZON.StopIntent': function () {
...
const helper = {
	...
	message += pause2s + promptToSayYesNo ;
